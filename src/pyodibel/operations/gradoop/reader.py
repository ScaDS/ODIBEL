...
try:
    ...
    plt = None  # Optional dependency for plotting

from pyodibel.operations.gradoop.cluster import Cluster
from pyodibel.operations.gradoop import graph_construction
from pyodibel.operations.gradoop.graph_construction import build_graph
from pyodibel.operations.gradoop.entity import Entity
from pyodibel.operations.gradoop import famer_constant
...
def read_vertex_csv_file(vertex_path, entity_dict, meta_dict_per_source, source_pair: set = set()):
    srcs = set()
    with (open(vertex_path, encoding="utf-8") as f):
...
if __name__ == "__main__":
    entities, cluster_graphs, cluster_list = read_csv_famer_data("/d/repo/MA Lerm/Datensammlungen/DS-C/DS-C0/SW_0.7")
    print(entities["5f3ba6223fd5b671b22055ef"].properties["<page title>"])