...
object TKGUtils {
  ...
  def countAllUniqueWindows(data: Dataset[CSVRow]): Long = {
  ...
  def countRevisionsPerPage(data: Dataset[CSVRow]): DataFrame = {
    import data.sparkSession.implicits._
  ...
  def countChangesPerPredicate(data: Dataset[CSVRow]): DataFrame = {
