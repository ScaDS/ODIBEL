...
from typing import Any, Iterator, Optional, Dict, Callable, Protocol, TYPE_CHECKING
from pydantic import BaseModel

# Backend imports - make optional to avoid breaking imports
if TYPE_CHECKING:
    from pyodibel.api.backend import ExecutionBackend, BackendType, BackendConfig, FormatHandler
else:
    try:
        from pyodibel.api.backend import ExecutionBackend, BackendType, BackendConfig, FormatHandler
    except ImportError:
        # Create minimal stubs if backend module doesn't exist
...
# TODO check join
# TODO check convert
# TODO check transform (mapping, restructuring, normalization)
...
class DataOperation(ABC):
    ...
    def _get_backend(self) -> Optional[ExecutionBackend]:
        ...
        if self._backend is None and self.config.backend:
            try:
                from pyodibel.api.backend import get_backend_registry
