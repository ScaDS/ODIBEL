...
def construct_query(subject_uri, properties):
    ...
    for prop in properties:
        if prop in PATH_NAV_PROPERTIES:
            ...
            where_lines.append(
                f"OPTIONAL {{ <{subject_uri}> <{prop}>/<http://www.w3.org/2000/01/rdf-schema#label> ?{label_var} . }}")
...
def run_query(query):
    ...
    if "xml" in content_type:
        ...
        print(f"Unrecognized content type: {content_type}")
    return graph


def save_graph(graph, name, category, folder):
    graph = sanitize_graph(graph)
    filename = os.path.join(BASE_DIRS[folder], category, f"{name}.nt")
...
def fetch_entity_data(uri, properties, category, recursive=False):
    ...
    for s, p, o in graph:
        s, p, o = sanitize_uri(s), sanitize_uri(p), sanitize_uri(o)
